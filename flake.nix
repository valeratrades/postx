  ...
  outputs = { nixpkgs, rust-overlay, flake-utils, pre-commit-hooks, v-parts, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        readme = (v-parts.readme-fw { inherit pkgs; last-supported-version = "nightly-1.85"; prj_name = manifest.name; root = ./.; loc = "395"; licenses = [{ name = "Blue Oak 1.0.0"; out_path = "LICENSE"; }]; badges = [ "msrv" "crates_io" "docs_rs" "loc" "ci" ]; }).combined;
      in
      {
        packages =
          let
